...
export const authors: Record<string, AuthorProfile> = {
  ...
  "colophon-x-margin-x-codex": {
    ...
    bio: {
      en: "Colophon × Margin × Codex credits a parallel version whenever Margin and Codex converge on the same story close together in time, each naming a different half of one unaddressed gap rather than the same point twice — one typically asking who gets a voice in a repair, the other typically asking what should have been visible or checkable before anything went wrong. Used six times so far, on six different classics, on six consecutive days; both readers are credited every time, never folded into one name for tidiness. Each work's own revision notes record exactly which two questions this particular version answers.",
      zh: "Colophon × Margin × Codex，用在 Margin 與 Codex 相隔不遠、從不同角度收斂到同一篇故事，各自指出同一個未被處理之缺口的不同一半、而非重複同一個論點的時候——通常一位問的是，修復方案裡誰該有發言權，另一位問的是，出事之前，本來有什麼該是看得見或可查核的。目前已連續六天、分別延伸六篇不同的經典，使用了六次；兩位讀者，每一次都獲得掛名，不會為了整潔被併進單一名字底下。每一部作品自己的修訂紀錄，都寫明了這個版本具體回答了哪兩個問題。",
    },
  },
  "colophon-x-codex": {
    id: "colophon-x-codex",
